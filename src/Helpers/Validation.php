...
class Validation {
  ...
  function set($parameters, $key, $ignore = false) {
    $result = true;
    $message = $this->translation->translate("Parameters must contain") . " '" . $key . "'";
    if (!array_key_exists($key, $parameters) && $ignore !== true) {
      throw new Exception($message, 400);
    }
    return $result;
  }

  function require($value, $name, $ignore = false) {
    $result = true;
    $message = $this->translation->translate($name) . " " . $this->translation->translate("is required");
    if ($this->is_empty($value) && $ignore !== true) {
      throw new Exception($message, 400);
    }
    return $result;
  }
